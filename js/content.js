...
const COURSE = {
  units: [
    ...
    /* ===================== UNIT — OPENCLAW ===================== */
    {
      id: 'upow',
      title: 'OpenClaw',
      subtitle: 'Run your own AI agent',
      color: '#364FC7',
      icon: '🦞',
      locked: false,
      lessons: [
        {
          id: 'upow1', title: 'OpenClaw: an agent you own', icon: '🦞', xp: 40,
          cards: [
            { type: 'info', title: 'The AI that actually does things', body: 'OpenClaw is a free, <b>open-source, self-hosted</b> AI agent. You run it on your own computer or server, and it connects an AI model to the messaging apps you already use — WhatsApp, Telegram, Signal, iMessage — and to real tools: your email, calendar, web browser and files. You message it like a contact and it <b>does the task</b>, not just talks about it.' },
            ...
            { type: 'info', title: 'It lives in your messages', body: 'The clever bit: instead of yet another app to learn, OpenClaw plugs into a messaging app you already use. You add it like a contact and just <b>text it</b> — “book me a table Friday”, “what’s on my calendar tomorrow”, “draft a reply to that email” — and it goes and does it. It even chats with other people’s AI agents.' },
            { type: 'choice', q: 'Why use a messaging app (WhatsApp, Telegram) as the interface?', options: ['It meets you where you already are — no new app to learn', 'Messaging is the only thing an AI is able to use', 'Texting somehow makes the underlying AI more powerful'], answer: 0, explain: 'It’s a usability choice — you already live in your messages. It doesn’t change what the AI can do or limit it to messaging; that’s just the front door.' },
            ...
            { type: 'info', title: 'Open-source: you can see inside it', body: 'Being <b>open-source</b> means the code is public — anyone can read it, run it, and change it. That matters a lot for an agent that can touch your email, files and accounts: you (or the wider community) can actually check what it does with your data, instead of trusting a closed box. The big paid assistants don’t give you that visibility.' },
            { type: 'choice', q: 'Why does “open-source” matter for an agent with access to your email and files?', options: ['Anyone can inspect exactly what it does with your data', 'It guarantees the software has no bugs', 'It means using it carries no risk at all'], answer: 0, explain: 'Transparency is the benefit — the code is open to inspection. Open-source doesn’t magically mean bug-free or risk-free; you still set it up carefully.' },
            { type: 'info', title: 'Self-hosted: it’s private by design', body: 'You run OpenClaw on <b>your own</b> computer or server. Your messages, your data and the agent itself live on hardware you control, rather than on some company’s cloud. For anyone wary about where their personal data ends up, that ownership is a big part of the appeal.' },
            { type: 'choice', q: 'A real benefit of OpenClaw being self-hosted?', options: ['The agent and your data run on hardware you control', 'It is always faster than any cloud AI', 'It removes the need for an AI model entirely'], answer: 0, explain: 'Self-hosting is about control and privacy. It still needs an LLM to do the thinking, and your own machine isn’t automatically faster than a big cloud data centre.' },
            { type: 'info', title: 'Fun fact: the name saga', body: 'It started life as <b>“Clawd”</b> — a cheeky nod to Anthropic’s Claude. Anthropic’s lawyers raised concerns about the name being too close to Claude, so it got renamed, eventually landing on <b>OpenClaw</b>. The “Claw” is the leftover wink at where it came from.' },
            { type: 'info', title: 'Why it’s a big deal — and the catch', body: 'OpenClaw went viral as “the AI that actually does things,” and OpenAI then hired its creator, <b>Peter Steinberger</b>, to build the next generation of personal agents — a sign the whole industry is racing toward an assistant that <i>does</i>, not just chats. The catch: an agent with real access needs <b>serious guardrails</b>. Remember, agents act on their assumptions and small errors compound — so you scope what it can touch and keep an eye on it.' },
            { type: 'choice', q: 'Who should realistically run OpenClaw today?', options: ['Technically-confident, curious folks happy to run a server — most people should just watch the space', 'Everyone — it’s a one-tap install for any phone', 'Nobody — it’s a gimmick that doesn’t actually work'], answer: 0, explain: 'It’s genuinely powerful but still a developer-ish setup, so it suits the keen and technical for now. It’s neither a one-tap consumer app yet, nor a gimmick — it’s a real glimpse of where personal agents are going.' }
          ]
        }
      ]
    },
    ...
    /* ===================== UNIT — SECOND BRAIN (OBSIDIAN) ===================== */
    {
      ...
      lessons: [
        ...
        {
          id: 'uobs6', title: 'Your AI second brain', icon: '🧠', xp: 35,
          cards: [
            { type: 'info', title: 'Now point AI at your notes', body: 'Here’s the high-level payoff of all this. Once your knowledge is captured in a vault, you can <b>point AI at it</b>. Plugins like <b>Smart Connections</b> and <b>Copilot for Obsidian</b> let you chat with your whole vault — the AI answers from <i>your</i> notes, not the open internet. “What did I learn on the Henderson job?” or “summarise everything I know about heat pumps” pulls straight from what you’ve actually written.' },
            { type: 'choice', q: 'Chatting with AI over your own notes is really an example of which technique?', options: ['Grounding — answering from material you supplied, not from training', 'Retraining the model on your personal data', 'A faster version of an internet search'], answer: 0, explain: 'It’s the grounding idea from AI Basics, applied to your own knowledge. Nothing gets retrained, and it isn’t searching the web — it’s reading your notes.' },
            { type: 'info', title: 'It retrieves first, then answers', body: 'It doesn’t re-read all 5,000 of your notes every time. When you ask something, it first <b>finds the handful of notes most relevant</b> to your question, then answers using just those. You’ll hear this called <b>RAG</b> (retrieval). It’s why it stays fast and accurate even with a huge vault.' },
            { type: 'choice', q: 'When you ask your vault a question, what does the AI do first?', options: ['Retrieves the most relevant notes, then answers from them', 'Reads every single note in full, every time', 'Writes an answer first, then looks for notes to match it'], answer: 0, explain: 'Retrieve-then-answer is the whole trick (RAG). It targets the relevant notes rather than ploughing through everything or making something up first.' },
            { type: 'info', title: 'It searches by meaning, not just words', body: 'Unlike a plain keyword search, it understands <b>meaning</b>. Ask about a “leaky tap” and it’ll surface a note where you wrote “dripping mixer”, because it knows they’re related. That’s called semantic search — and it’s why an AI second brain finds things your memory (and a basic search box) would miss.' },
            { type: 'truefalse', q: 'An AI second brain can find a relevant note even when it used different words than your search.', isTrue: true, explain: 'Semantic search matches on meaning, so “leaky tap” can surface a note about a “dripping mixer”. Keyword search can’t do that.' },
            { type: 'info', title: 'The two go-to plugins', body: 'Two main tools, slightly different jobs. <b>Smart Connections</b> works quietly in the background — as you write, it surfaces related notes in a sidebar, plus a chat. <b>Copilot for Obsidian</b> is the fuller assistant — chat with your whole vault, generate summaries, run AI tasks. Both are free and both can use local or cloud models.' },
            { type: 'choice', q: 'You want to actively chat with your whole vault and generate summaries. Best fit?', options: ['Copilot for Obsidian', 'The graph view', 'Daily Notes'], answer: 0, explain: 'Copilot is built for chatting with and summarising your vault. The graph just shows links, and Daily Notes is for capturing — neither chats with your notes.' },
            { type: 'truefalse', q: 'Because it answers from your real notes, an AI second brain makes far less up than asking it cold.', isTrue: true, explain: 'Grounding keeps it to your actual material, so there’s much less room to invent.' },
            { type: 'info', title: 'Only as good as your notes', body: 'The catch: it can only answer from what you’ve actually written. Thin or messy notes give thin answers — garbage in, garbage out. And it can still misread or miss something, so for anything that matters, verify against the note itself. The AI is a brilliant librarian for your knowledge; it doesn’t create knowledge you never captured.' },
            { type: 'choice', q: 'A realistic limit of an AI second brain?', options: ['It’s only as good as the notes you’ve actually written', 'It can’t be wrong, because it’s using your own notes', 'It removes any need to take notes in the first place'], answer: 0, explain: 'It works from your captured knowledge — so it can’t answer what you never wrote down, and it can still misread. It makes good notes more powerful; it doesn’t replace them.' },
            { type: 'info', title: 'Private, and plugged into Claude', body: 'Because your notes are local files, you can run this with a <b>local model</b> (via a tool like Ollama) so notes and AI both stay on your machine — private by design, ideal for sensitive job or client records. Or go the other way and connect <b>Claude via MCP / Connectors</b> (from the Claude unit) so it can read, search and even update your vault as a live workspace. Either way: a private, searchable AI expert on everything <i>you</i> know.' }
          ]
        }
      ]
    },
